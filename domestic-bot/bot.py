...
def signal_handler(sig, frame):
    ...
    # Schedule the bot's close in the event loop
...
async def shutdown_monitor():
    """Check for shutdown signals every 2 seconds"""
    if check_for_shutdown_signal():
        # Stop all tasks
        routine_function.cancel()
        shutdown_monitor.cancel()
        
        # Log the shutdown
        logger.info("Gracefully shutting down the bot")
        
        # Close the Discord connection
        await bot.close()

@shutdown_monitor.before_loop
async def before_shutdown_monitor():
    """Wait for bot to be ready before starting shutdown_monitor loop."""
    await bot.wait_until_ready()
